...
pkg/bin/windows_amd64/terraform-provider-azurerm.exe:
	...
	GOFLAGS=-mod=vendor GOOS=windows GOARCH=amd64 go build -o pkg/bin/windows_amd64/terraform-provider-azurerm.exe

pkg/bin/linux_amd64/terraform-provider-azurerm:
	mkdir -p pkg/bin/linux_amd64
	GOFLAGS=-mod=vendor GOOS=linux GOARCH=amd64 go build -o pkg/bin/linux_amd64/terraform-provider-azurerm

pkg/bin/darwin_amd64/terraform-provider-azurerm:
	mkdir -p pkg/bin/darwin_amd64
	GOFLAGS=-mod=vendor GOOS=darwin GOARCH=amd64 go build -o pkg/bin/darwin_amd64/terraform-provider-azurerm
